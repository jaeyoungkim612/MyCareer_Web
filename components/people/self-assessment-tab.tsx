...
export function SelfAssessmentTab({ empno: propEmpno, readOnly = false }: SelfAssessmentTabProps = {}) {
  ...
  const fetchAssessments = async () => {
    ...
    try {
      ...
      if (midError || !mid) {
        console.log("🔄 PeopleSelfAssessment: Trying mid assessment with original empno:", empno)
      ...
      if (finalError || !final) {
        console.log("🔄 PeopleSelfAssessment: Trying final assessment with original empno:", empno)
        const result = await supabase
          .from("people_final_assessments")
          .select("*")
          .eq("empno", empno)
  ...
  const handleReopenMid = () => handleReopen("people_mid_assessments", "중간평가", () => { setIsEditingMid(true); setTabValueMid("edit") })
  const handleReopenFinal = () => handleReopen("people_final_assessments", "기말평가", () => { setIsEditingFinal(true); setTabValueFinal("edit") })
  // 임시저장/제출 (upsert로 저장)
