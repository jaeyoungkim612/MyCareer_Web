...
export default function ExpertisePlanTab({ empno, readOnly = false }: ExpertisePlanTabProps = {}) {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      ...
      console.log(`🔍 QualityPlanTab: loadUserData - readOnly=${readOnly}, empno=${empno}, targetEmpno=${targetEmpno}`)
      
      if (!targetEmpno) {
        if (readOnly) {
          console.warn('⚠️ QualityPlanTab: readOnly 모드인데 empno가 전달되지 않았습니다.')
        }
        setIsLoading(false)
        return
      }
