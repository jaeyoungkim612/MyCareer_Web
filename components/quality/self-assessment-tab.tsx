...
interface QualitySelfAssessmentTabProps {
  empno?: string
  readOnly?: boolean
}

export default function QualitySelfAssessmentTab({ empno: propEmpno, readOnly = false }: QualitySelfAssessmentTabProps = {}) {
  ...
  const fetchAssessments = async () => {
    ...
    try {
      ...
      console.log(`🔧 QualitySelfAssessment: Normalizing empno: ${empno} → ${normalizedEmpno}`)
      ...
      if (midError || !mid) {
        console.log("🔄 QualitySelfAssessment: Trying mid assessment with original empno:", empno)
      ...
      if (finalError || !final) {
        console.log("🔄 QualitySelfAssessment: Trying final assessment with original empno:", empno)
        const result = await supabase
          .from("quality_final_assessments")
          .select("*")
          .eq("empno", empno)
