...
export default function IndustryPage() {
  ...
  const [industryScore, setIndustryScore] = useState(getScoreByCategory("Industry & TL"))
  const [tempScores, setTempScores] = useState({
    current: industryScore?.currentScore || 0,
    goal: industryScore?.targetScore || 0,
  })
    ...
    const loadUserScores = async () => {
      if (user?.empno) {
        ...
        try {
          ...
          const updatedScore = getScoreByCategory("Industry & TL")
          setIndustryScore(updatedScore)
          setTempScores({
  ...
  const handleSaveScores = async () => {
    if (industryScore && user?.empno) {
      try {
        await updateScoreByCategory("Industry & TL", tempScores.current, tempScores.goal)
        const updatedScore = getScoreByCategory("Industry & TL")
        setIndustryScore(updatedScore)
      } catch (error) {
  ...
  const handleCancelScores = () => {
    setTempScores({ current: industryScore?.currentScore || 0, goal: industryScore?.targetScore || 0 })
    setIsEditingScore(false)
  }

  // industryScore 변경 시 tempScores 동기화
  useEffect(() => {
    if (industryScore) {
      setTempScores({
        current: industryScore.currentScore,
        goal: industryScore.targetScore,
      })
    }
  }, [industryScore])
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-6">
                ...
                <div className="flex items-baseline gap-1 mr-6">
                  <span className="text-lg font-bold text-orange-600 dark:text-orange-400">{industryScore?.currentScore}</span>
                  <span className="text-sm font-medium text-orange-400/70">/ {industryScore?.targetScore}</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Dialog>
                  ...
                  <DialogContent className="max-w-lg">
                    <DialogHeader>
                      <DialogTitle className="text-orange-700 dark:text-orange-300">Industry 점수 예시</DialogTitle>
                    </DialogHeader>
